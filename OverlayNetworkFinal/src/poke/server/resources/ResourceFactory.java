...
public class ResourceFactory {
	...
	public static void initialize(ServerConf cfg) {
		try {
			...
			logger.info("Resource factory is inialized..");
		} catch (Exception e) {
	...
	public Resource resourceInstance(Header.Routing route) {
		ResourceConf rc = cfg.findResourceById(route.getNumber());
		if (rc == null)
			return null;
	 ...
	 * @return
	 */
	public Node serverInstance(String nodeid) {
		
		try {
			Node node = cfg.findNodeById(nodeid);
			if (node == null)
				return null;
