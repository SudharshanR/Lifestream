			...
			while (true) {
				...
				try {
					...
					if (msg instanceof Request) {
						...
						logger.info("req.getHeader(). ::::::"+req.getHeader());
						logger.info("req.getHeader().getRoutingId() ::::::"+req.getHeader().getRoutingId());
						Resource rsc = ResourceFactory.getInstance()
								.resourceInstance(
										req.getHeader().getRoutingId());


						// reply back to requestor.
						Response reply = null;
						if (rsc == null) {
						...
						if (reply == null) {
							//this means XXXResource was not able to process the message
							//in other words, file(image) is not found on this server

							// TODO : append visited node to message						
							// implement the code to add visited node to message.							
							// req.getHeader().addVisitedNode(ServerConf.getServerid()); 	
							
											// this may require us to change the protobuf 
											// message structure and Server.conf resource.

							// forward this request to neighbor node (except the requestor)

						}else{
							//this means XXXResource successfuly processed message
							//in other words, file(image) is found on this server

							//- reply back to the most recent requestor
							//- this reply will travel the visited node backward.
							// TODO : implement the logic to reply the message backward.
							//  ** before passing a message to next neighbor,
							//	** we need to append the "visited node" to header
							//	** (we may also need to save processed message in the DB 
							//	** because we will drop duplicated request)
